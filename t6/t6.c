...
static int find_index_of_item(char *name, int current_working_dir) {
	int i, curr_block_num;
	for (i = 1; i < INDEX_SIZE; i++) {
		curr_block_num = memory[current_working_dir].content.index[i];
		if (strcmp(memory[curr_block_num].content.fd.name, name) == 0) {
			return memory[curr_block_num].content.fd.block_ref;
		}
	}
	return -1;
}
...
static int fs_mknod(const char *name, mode_t mode, dev_t dev) {
	...
	path = strdup(name);
	token = strsep(&path, "/");
	log_msg("RESETTING PATH TO %s\n", path);
	int dir = working_dir;
	log_msg("DIR = %d\n", dir);
	char *modified_token;
	for (i = 1; i < path_count; i++) {
		token = strsep(&path, "/");
		modified_token = malloc(strlen(token) + 2);
		strcpy(modified_token, "/");
		strcat(modified_token, token);
		// find dir token in current directory
		log_msg("LOOKING UP INDEX OF ITEM %s IN DIR %d\n", modified_token, dir);
		dir = find_index_of_item(modified_token, dir);
		log_msg("INDEX IS %d\n", dir);
		if (dir == -1) {
			log_msg("ERROR: COULDN'T FIND INDEX OF ITEM %s\n", name);
			return -1;
		}
	}
	log_msg("DIR = %d\n", dir);
	...
	for (i = 1; i < INDEX_SIZE && !found; i++) {
		if (memory[dir].content.index[i] == 0) {
			found = 1;
			memory[dir].content.index[i] = file_blockNumber;
		}
	}
	if (!found) {
	...
	log_msg("Created file \"%s\" in block %d with data node in block %d. bitvector[%d] offset %d. PARENT DIR BLOCK %d\n", 
		name, file_blockNumber, data_blockNumber, index, file_offset, dir);
	//
    char fpath[PATH_MAX];
...
static int create_dir(const char *path, mode_t mode) {
	...
	memory[index_blockNumber].content.index[0] = working_dir;
...
static int fs_unlink(const char *name) {
	log_msg("test %s\n", "fs_unlink");
	int curr_block_num, found;
	if (FS_DATA->block_num > 0) {
		// if the file is a child of a directory being deleted, we are passed the file's memory block number through fs_get_context()
		found = 1;
		curr_block_num = FS_DATA->block_num;
	}
	else {
		int i;
		found = 0;
...
static int fs_rmdir(const char *name) {
	log_msg("FS_RMDIR ON DIR %s\n", name);
	int found, curr_block_num;
	if (FS_DATA->block_num > 0) {
		// if the dir is a child of a directory being deleted, we are passed the dir's memory block number through fs_get_context()
		log_msg("BLOCK_NUM IS GIVEN: %d\n", FS_DATA->block_num);
		found = 1;
		curr_block_num = FS_DATA->block_num;
		FS_DATA->block_num = 0;
	} 
	...
	if (found) {
		log_msg("FOUND DIR IN WORKING DIR AT INDEX %d\n", curr_block_num);
		// Delete it:
		...
		if (!(FS_DATA->is_child)) {
			...
			log_msg("DIR IS A FIRST-LEVEL PARENT\n");
			// set index in dir's parent's index array to 0. 
			found = 0;
			int parent_block = working_dir;
			log_msg("DIR'S PARENT IS IN BLOCK %d\n", parent_block);
			for (i = 1; i < INDEX_SIZE && !found; i++) {
			...
			if (!found) {
				...
				log_msg("FOUND DIR'S INDEX IN PARENT DIRECTORY: %d\n", i);
			}

			
			int index_block = memory[blockNumber].content.fd.block_ref;
			// remove directory's index block from the memory array
			bitvector_index = index_block / 8;
			bitvector_offset = index_block % 8;
			mask = 0x80;
			for (i = 0; i < bitvector_offset; i++) {
				mask /= 2;
			}
...
static int fs_getattr(const char *name, struct stat *st) {
	...
	int found = 0;
	for (i = 1; i < INDEX_SIZE && !found; i++) {
		curr_block_num = memory[dir].content.index[i];
...
static int fs_open(const char *path, struct fuse_file_info *fi) {
	...
	char *token;
	token = strsep(&new_path, "/");
	int path_count = 0;
	while ((token = strsep(&new_path, "/"))) {
		path_count++;
	}
	int i;
    ...
    int block_num;
    for (i = 1; i < INDEX_SIZE; i++) {
    	block_num = memory[dir].content.index[i];
